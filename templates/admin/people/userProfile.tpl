...
<p><a href="{url op="editUser" path=$user->getUserId()}" class="action">{translate key="admin.people.editProfile"}</a></p>
...
{section name=role loop=$userRoles}
	<li>{translate key=$userRoles[role]->getRoleName()} <a href="{url op="unEnroll" path=$userRoles[role]->getRoleId() userId=$user->getUserId()}" onclick="return confirm('{translate|escape:"jsparam" key="admin.people.confirmUnenroll"}')" class="action">{translate key="admin.people.unenroll"}</a></li>
{/section}
</ul>
