...
{if $versionId}
	<ul class="menu">
		<li class="current"><a href="{url op="editVersion" path=$archiveId|to_array:$versionId}" class="action">{translate key="rt.admin.versions.metadata"}</a></li>
		<li><a href="{url op="contexts" path=$archiveId|to_array:$versionId}" class="action">{translate key="rt.contexts"}</a></li>
	</ul>
{/if}

<br />

<form class="pkp_form" action="{if $versionId}{url op="saveVersion" path=$archiveId|to_array:$versionId}{else}{url op="createVersion" path="save"}{/if}" method="post">
