...
{if $contextId}
	<ul class="menu">
		<li class="current"><a href="{url op="editContext" path=$archiveId|to_array:$versionId:$contextId}" class="action">{translate key="rt.admin.contexts.metadata"}</a></li>
		<li><a href="{url op="searches" path=$archiveId|to_array:$versionId:$contextId}" class="action">{translate key="rt.searches"}</a></li>
	</ul>
{/if}
		...
		<td class="value">
			<table width="100%" class="data">
				...
				<tr valign="top">
					...
					<td><label for="defineTerms">{translate key="rt.admin.contexts.options.defineTerms"}</label></td>
				</tr>
				<tr valign="top">
					<td><input type="checkbox" name="citedBy" id="citedBy" {if $citedBy}checked="checked"{/if} /></td>
					<td><label for="citedBy">{translate key="rt.admin.contexts.options.citedBy"}</label></td>
				</tr>
			</table>
		</td>
	</tr>
...
<p><input type="submit" value="{translate key="common.save"}" class="button defaultButton" /> <input type="button" value="{translate key="common.cancel"}" class="button" onclick="document.location.href='{url op="contexts" path=$archiveId|to_array:$versionId escape=false}'" /></p>
