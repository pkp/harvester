...
/**
 * @file classes/sortOrder/SortOrderSettingsDAO.inc.php
 *
 * Copyright (c) 2005-2012 Alec Smecher and John Willinsky
 * Distributed under the GNU GPL v2. For full terms see the file docs/COPYING.
 *
 * @class SortOrderSettingsDAO
 * @ingroup sort_orders
 *
...
class SortOrderSettingsDAO extends DAO {
	...
	/**
	 * Retrieve a sort order setting value.
	 * @param $sortOrderId int
	 * @param $name string
	 * @param $locale string optional
	 * @return mixed
	 */
	function &getSetting($sortOrderId, $name, $locale = null) {
		$cache =& $this->_getCache($sortOrderId);
	...
	function _cacheMiss(&$cache, $id) {
		$settings =& $this->getSortOrderSettings($cache->getCacheId());
	...
	/**
	 * Retrieve and cache all settings for a sort order.
	 * @param $sortOrderId int
	 * @return array
	 */
	function &getSortOrderSettings($sortOrderId) {
		$sortOrderSettings = array();

		$result =& $this->retrieve(
			'SELECT setting_name, setting_value, setting_type, locale FROM sort_order_settings WHERE sort_order_id = ?', $sortOrderId
		);

		while (!$result->EOF) {
			$row =& $result->getRowAssoc(false);
			$value = $this->convertFromDB($row['setting_value'], $row['setting_type']);
			if ($row['locale'] == '') $sortOrderSettings[$row['setting_name']] = $value;
			else $sortOrderSettings[$row['setting_name']][$row['locale']] = $value;
			$result->MoveNext();
		}
		$result->Close();
		unset($result);
		...
		$cache =& $this->_getCache($sortOrderId);
		$cache->setEntireCache($sortOrderSettings);

		return $sortOrderSettings;
	}

	/**
	 * Add/update a sort order setting.
	 * @param $sortOrderId int
	 * @param $name string
	 * @param $value mixed
	 ...
	 */
	function updateSetting($sortOrderId, $name, $value, $type = null, $isLocalized = false) {
		$cache =& $this->_getCache($sortOrderId);
		$cache->setCache($name, $value);

		$keyFields = array('setting_name', 'locale', 'sort_order_id');
		
		if (!$isLocalized) {
			$value = $this->convertToDB($value, $type);
			$this->replace('sort_order_settings',
				array(
			...
			if (is_array($value)) foreach ($value as $locale => $localeValue) {
				$this->update('DELETE FROM sort_order_settings WHERE sort_order_id = ? AND setting_name = ? AND locale = ?', array($sortOrderId, $name, $locale));
				if (empty($localeValue)) continue;
				$type = null;
				$this->update('INSERT INTO sort_order_settings
					(sort_order_id, setting_name, setting_value, setting_type, locale)
					VALUES (?, ?, ?, ?, ?)',
					array(
						$sortOrderId, $name, $this->convertToDB($localeValue, $type), $type, $locale
					)
				);
			}
		}
	...
	/**
	 * Delete a sort order setting.
	 * @param $sortOrderId int
	 * @param $name string
	 */
	function deleteSetting($sortOrderId, $name, $locale = null) {
		$cache =& $this->_getCache($sortOrderId);
		$cache->setCache($name, null);

		$params = array($sortOrderId, $name);
		$sql = 'DELETE FROM sort_order_settings WHERE sort_order_id = ? AND setting_name = ?';
		if ($locale !== null) {
			$params[] = $locale;
			$sql .= ' AND locale = ?';
		}
	...
	/**
	 * Delete all settings for a sort order.
	 * @param $sortOrderId int
	 */
	function deleteSettingsBySortOrder($sortOrderId) {
		$cache =& $this->_getCache($sortOrderId);
		$cache->flush();

		return $this->update(
				'DELETE FROM sort_order_settings WHERE sort_order_id = ?', $sortOrderId
		);
	}
	...
	/**
	 * Install sort order settings from an XML file.
	 * @param $sortOrderId int ID of sort order for settings to apply to
	 ...
	 */
	function installSettings($sortOrderId, $filename, $paramArray = array()) {
	...
	/**
	 * Install locale field only sort order settings from an XML file.
	 * @param $sortOrderId int ID of sort order for settings to apply to
	 * @param $filename string Name of XML file to parse and install
	 * @param $paramArray array Optional parameters for variable replacement in settings
	 * @param $locale string locale id for which settings will be loaded
	 */
	function reloadLocalizedDefaultSettings($sortOrderId, $filename, $paramArray, $locale) {
		$xmlParser = new XMLParser();
		$tree = $xmlParser->parse($filename);
