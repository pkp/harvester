 ...
 *
 * Form for site admins to view and modify static pages.
 *
 */
...
class StaticPagesEditForm extends Form {
	 ...
	 * Constructor
	 * @param $plugin object
	 * @param $staticPageId int optional
	 */
	function StaticPagesEditForm(&$plugin, $staticPageId = null) {
		...
		$this->addCheck(new FormValidatorCustom($this, 'pagePath', 'required', 'plugins.generic.staticPages.duplicatePath', array(&$this, 'checkForDuplicatePath'), array($staticPageId)));
		$this->addCheck(new FormValidatorPost($this));

	}

	/**
	 * Custom FormValidator to ensure no duplicate page paths are created.
	 * @param $pagePath string
	 * @param $staticPageId int
	 */
	function checkForDuplicatePath($pagePath, $staticPageId) {
		$staticPageDAO =& DAORegistry::getDAO('StaticPagesDAO');

		return !$staticPageDAO->duplicatePathExists($pagePath, $staticPageId);
	}
	...
	function addTinyMCE() {
		...
		<script language="javascript" type="text/javascript">
			tinyMCE.init({
			mode : "textareas",
			plugins : "safari,spellchecker,style,layer,table,save,advhr,advimage,advlink,emotions,iespell,inlinepopups,insertdatetime,preview,media,searchreplace,print,contextmenu,paste,directionality,fullscreen,noneditable,visualchars,nonbreaking,xhtmlxtras,template,pagebreak,jbimages",
			...
			theme_advanced_buttons4 : "cut,copy,paste,pastetext,pasteword,separator,styleprops,|,spellchecker,cite,abbr,acronym,del,ins,attribs,|,visualchars,nonbreaking,template,blockquote,pagebreak,print,separator,jbimages",
			theme_advanced_toolbar_location : "top",
			theme_advanced_toolbar_align : "left",
			theme_advanced_statusbar_location : "bottom",
			relative_urls : false,
			document_base_url : "'. Request::getBaseUrl() .'/'.$publicFileManager->getSiteFilesPath() .'/",
			theme : "advanced",
