...
class CustomBlockPlugin extends BlockPlugin {
	...
	function manage($verb, $args) {
		...
		$form = new CustomBlockEditForm($this);
		...
		switch ($verb) {
			...
			case 'save':
				...
				if ($form->validate()) {
					$form->save();
					$pageCrumbs[] = array(Request::url('admin', 'plugins'), 'admin.plugins');
					$templateMgr->assign(array(
						'currentUrl' => Request::url(null, null, array($this->getCategory(), $this->getName(), 'edit')),
						'pageTitleTranslated' => $this->getDisplayName(),
						'pageHierarchy' => $pageCrumbs,
						'message' => 'plugins.generic.customBlock.saved',
						'backLink' => Request::url('admin', 'plugins'),
	...
	function getContents(&$templateMgr) {
		$templateMgr->assign('customBlockContent', $this->getSetting('blockContent'));
